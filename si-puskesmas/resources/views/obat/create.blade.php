<x-dashboard-layout>
    <section class="flex flex-col gap-8">
        <div class="flex gap-2 items-center">
            ...
            <p class="text-xl">/</p>
            <p class="text-xl">Tambah Obat</p>
        </div>

        <form action="{{route('obat.store')}}" class="w-80 flex flex-col gap-6" method="POST">
            @csrf
            <div>
                <x-input-label :value="__('Nama Obat')" />
                <x-text-input class="block mt-1 w-full" type="text" name="nama_obat" required />
            </div>
            <div>
                <x-input-label :value="__('Stok')" />
                <x-text-input class="block mt-1 w-full" type="number" name="stok" required />
            </div>
            <div>
            ...
            <div>
                ...
                <x-text-input class="block mt-1 w-full" type="date" name="kadaluwarsa" required />
            </div>
            <div>
