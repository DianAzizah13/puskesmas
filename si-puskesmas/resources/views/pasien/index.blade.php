<x-dashboard-layout>
    <section class="flex flex-col gap-4">
        <h1 class="text-xl">Daftar Pasien</h1>

        <div class="justify-between flex mt-6">
            <form method="GET" action="{{route('pasien.index')}}">
                <x-text-input class="block w-full" type="text" name="query" placeholder="Cari Data (NIK/Nama) ..." />
            </form>
            <div class="flex gap-2">
                <x-bladewind::button href="{{route('pasien.print', ['query'=>$query])}}" color="green"
                    class="w-40 rounded-md text-center" size="small" tag="a">Cetak Data</x-bladewind::button>
                <x-bladewind::button href="{{route('pasien.create')}}" class="w-60 rounded-md text-center" size="small"
                    tag="a">Tambah Pasien</x-bladewind::button>
            </div>
        </div>
            ...
            <x-bladewind::table has_border="true" striped="true" celled="true">
                ...
                <tr>
                    ...
                    <td class="inline-flex items-center gap-2 w-full">
                        ...
                        <x-bladewind::button tag="a" class="inline-flex items-center p-2 px-3 text-center rounded-md"
                            href="{{ route('rekam-medis.user-index', $d->id) }}">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 24 24">
                                <path fill="currentColor"
